# Memorus — AI 智能体的自适应记忆引擎

> 基于 mem0 + ACE 智能层：自动学习、自动遗忘、自动召回

Memorus 在 [mem0](https://github.com/mem0ai/mem0) 基础上构建了 **ACE（自适应上下文引擎）** —— 一套自动提炼、去重、衰退和检索知识的流水线，让你的 AI 智能体记住重要的、遗忘无关的。

---

## 为什么选择 Memorus：本地优先的智能记忆
...
### Memorus 的解法：Local-First

Memorus 的核心设计原则是**本地优先**——所有核心能力默认在本地完成，零外部依赖。LLM 是可选增强，不是必需前提。
...
```python
# 不启用 ACE 时，Memorus 是 mem0 的透明代理，行为完全一致
m = Memory()
...
# 迁移后 (Memorus, ACE 关闭 — 行为完全一致，零开销)
from memorus import Memory
m = Memory(config={"vector_store": {"provider": "qdrant", ...}})
m.add("fact", user_id="u1")
results = m.search("query", user_id="u1")

# 迁移后 (Memorus, ACE 开启 — 完整智能管道)
from memorus import Memory
m = Memory(config={
    "ace_enabled": True,
...
`config` 是一个包含 mem0 键和 ACE 键的字典。Memorus 自动分离——ACE 键（`ace_enabled`、`reflector`、`curator`、`decay`、`retrieval`、`privacy`、`integration`、`daemon`）由 ACE 引擎消费；其余全部转发给 mem0 后端。
...
Memorus 使用与 mem0 相同的配置字典格式，附加 ACE 专用键。所有 ACE 键均可选——默认值针对通用场景优化。
...
memorus learn "总是使用 -v 参数"

# 原始写入（跳过 Reflector）
memorus learn "原始事实" --raw
...
memorus forget <memory-id> --yes   # 跳过确认
...
Memorus 将每条知识 Bullet 标记为一个**类型**和一个**分区**，实现结构化组织。
...
1. 已安装 `litellm`（`pip install memorus[llm]`）
