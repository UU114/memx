...
So that MemX automatically integrates with my product's lifecycle
...
### Background
MemX 通过三个 Hook 点与宿主 AI 产品集成：PreInferenceHook（用户输入前自动召回记忆）、PostActionHook（工具调用后自动蒸馏）、SessionEndHook（会话结束时兜底蒸馏 + Decay sweep）。IntegrationManager 是这些 Hook 的注册中心和生命周期管理器。本 Story 定义抽象层，具体 CLI 实现在 STORY-033/034。
...
### Components
- `memx/integration/__init__.py` — 包入口，导出公共 API
- `memx/integration/hooks.py` — BaseHook, PreInferenceHook, PostActionHook, SessionEndHook, ContextInjection, ToolEvent
- `memx/integration/manager.py` — IntegrationManager
...
- `memx/config.py:IntegrationConfig` — 已定义（auto_recall, auto_reflect, sweep_on_exit）
- `memx/memory.py:Memory` — search() 和 add() 方法
...
- [ ] `memx/integration/hooks.py` 定义全部抽象类和数据类
- [ ] `memx/integration/manager.py` 实现 IntegrationManager
