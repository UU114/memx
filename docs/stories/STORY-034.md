...
### Components
- `memx/integration/cli_hooks.py` — CLIPostActionHook, CLISessionEndHook
...
- `memx/integration/hooks.py` — PostActionHook, SessionEndHook（STORY-032）
- `memx/pipeline/ingest.py:IngestPipeline` — add 路径
- `memx/engines/decay/engine.py:DecayEngine.sweep()` — 衰退扫描
- `memx/config.py:IntegrationConfig` — auto_reflect, sweep_on_exit
...
- [ ] `memx/integration/cli_hooks.py` 实现 CLIPostActionHook + CLISessionEndHook
- [ ] 信号处理逻辑实现
- [ ] 异步蒸馏不阻塞主流程（测试验证）
