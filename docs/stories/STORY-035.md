...
### Background
Integration Layer 是 MemX 与宿主 AI 产品的核心连接点。Hook 的可靠性直接影响用户体验——PreInference 失败意味着 AI 丢失记忆，PostAction 失败意味着知识不被学习，SessionEnd 失败意味着衰退不执行。本 Story 对全部 Integration 组件进行系统测试。
...
- `memx/integration/` — 全部 STORY-032/033/034 实现
- `memx/engines/decay/engine.py` — DecayEngine（用于 SessionEnd 测试）
