...
### Background
DaemonClient 是 MemXDaemon 的客户端封装，供 CLI Hook 和 CLI 命令使用。它封装了 IPC 连接细节（Named Pipe / Unix Socket），提供类型安全的 API，并自动探测 Daemon 是否运行。当 Daemon 不可用时抛出可捕获的 `DaemonUnavailableError`，供上层进行降级处理。
...
### Components
- `memx/daemon/client.py` — DaemonClient
- `memx/daemon/ipc.py` — IPCTransport 抽象层
...
class NamedPipeTransport(IPCTransport):
    ...
    def __init__(self, pipe_name: str = r"\\.\pipe\memx-daemon"): ...

class UnixSocketTransport(IPCTransport):
    """Unix Socket transport."""
    def __init__(self, socket_path: str = "~/.memx/daemon.sock"): ...
...
- `memx/daemon/server.py:DaemonRequest, DaemonResponse` — 共享数据类
- `memx/config.py:DaemonConfig` — socket_path, enabled 等配置
...
- [ ] `memx/daemon/ipc.py` 实现 IPCTransport + NamedPipeTransport + UnixSocketTransport
- [ ] `memx/daemon/client.py` 实现 DaemonClient
