...
So that MemX always works regardless of Daemon status
...
### Background
Daemon 是性能优化组件，不是必要依赖。当 Daemon 未启动、崩溃或不响应时，MemX 必须透明降级到直接调用 Memory 库模式（冷启动但功能完整）。降级和恢复对用户完全透明，仅通过日志通知。
...
降级逻辑集成到 `memx/memory.py` 的 `MemXMemory` 类：
...
- `memx/memory.py:MemXMemory` — 主 Memory 类
- `memx/daemon/client.py:DaemonClient` — IPC 客户端（STORY-038）
- `memx/config.py:DaemonConfig` — enabled 配置
