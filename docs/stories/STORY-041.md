...
So that I can see what MemX has learned and search my memories
...
### Background
CLI 是 MemX 面向用户的主要交互界面。`memx status` 提供知识库概览（总数、分布、健康状态），`memx search` 提供交互式混合检索。两者是最基础的用户可见功能。使用 Click 框架实现。

### Scope
**In scope:**
- `memx status` — 知识库统计命令
- `memx search <query>` — 混合检索命令
- Click 框架搭建
- `--json` 输出格式支持
...
- [ ] `memx status` 输出：记忆总数、section 分布、knowledge_type 分布、平均 decay_weight、ACE 模式（on/off）
- [ ] `memx status --json` 输出 JSON 格式
- [ ] `memx search <query>` 使用 RetrievalPipeline 混合检索
- [ ] 搜索结果格式化输出：score、content 摘要、knowledge_type、tags
- [ ] `memx search <query> --json` 输出 JSON 格式
- [ ] `memx search <query> --limit N` 限制结果数量（默认 5）
- [ ] 无记忆时 `memx status` 显示友好提示
- [ ] 搜索无结果时显示友好提示
- [ ] `memx --version` 显示版本号
- [ ] `memx --help` 显示帮助信息
...
### Components
- `memx/cli/__init__.py` — 包入口
- `memx/cli/main.py` — Click CLI app
...
def status(ctx, as_json: bool):
    ...
    stats = {
        ...
        "ace_enabled": True,
        "sections": _count_by(all_memories, "memx_section"),
        "knowledge_types": _count_by(all_memories, "memx_knowledge_type"),
        "avg_decay_weight": _avg_field(all_memories, "memx_decay_weight"),
    }
...
- `memx/memory.py:Memory` — get_all(), search() API
- `memx/config.py:MemXConfig` — 配置加载

### Edge Cases
- 空知识库 → status 显示 "No memories yet. Use `memx learn` to add knowledge."
...
- [ ] `memx/cli/main.py` 实现 status + search 命令
- [ ] pyproject.toml 注册 entry point
- [ ] `memx --version` 和 `memx --help` 正常工作
- [ ] `--json` 输出格式测试
- [ ] mypy --strict 通过
- [ ] ruff check 通过
